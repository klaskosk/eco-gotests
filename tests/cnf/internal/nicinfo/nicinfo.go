...
var (
	driverRegex           = regexp.MustCompile(`^driver: (.+)$`)
	versionRegex          = regexp.MustCompile(`^version: (.+)$`)
	firmwareVersionRegex  = regexp.MustCompile(`^firmware-version: (.+)$`)
	ptpHardwareClockRegex = regexp.MustCompile(`^PTP Hardware Clock: (\d+)$`)
)
