...
var _ = Describe("PTP Process Restart", Label(tsparams.LabelProcessRestart), func() {
	...
	It("should create a new ptp4l process after killing a ptp4l process that is not related to the "+
		...
		isPtpAtLeast420, err := version.IsVersionStringInRange(RANConfig.Spoke1OperatorVersions[ranparam.PTP], "4.20", "4.99")
		Expect(err).ToNot(HaveOccurred(), "Failed to check if PTP operator version is at least 4.20")
		...
		for _, nodeInfo := range nodeInfoMap {
			...
			if isPtpAtLeast420 {
				By("waiting for the HOLDOVER event to be received after killing the ptp4l process for 4.20+")
				filter := events.All(
					events.IsType(eventptp.PtpStateChange),
					events.HasValue(events.WithSyncState(eventptp.HOLDOVER), events.ContainingResource(string(iface.Master))),
				)
				err = events.WaitForEvent(
					eventPod, startTime, 3*time.Minute, filter, events.WithoutCurrentState(true))
				Expect(err).ToNot(HaveOccurred(), "Failed to wait for holdover event on node %s", nodeInfo.Name)
			} else {
			...
			if isPtpAtLeast420 {
				By("verifying that no FREERUN event is received after killing the ptp4l process")
				filter := events.All(
					events.IsType(eventptp.PtpStateChange),
					events.HasValue(events.WithSyncState(eventptp.FREERUN), events.ContainingResource(string(iface.Master))),
				)
				// We really want to check that no FREERUN event is received from startTime until the
				// time of the LOCKED event, which should be about now. Just to be safe, we ensure that
				// no event shows up within one minute of now. This could probably be lowered but we err
				// on the side of caution.
				err = events.WaitForEvent(
					eventPod, startTime, time.Minute, filter, events.WithoutCurrentState(true))
				Expect(err).To(HaveOccurred(), "Expected FREERUN event to not be received on node %s", nodeInfo.Name)
			}
