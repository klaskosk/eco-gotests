...
- `options`: Optional parameters to customize log retrieval, such as specifying a container name using `WithContainer`.
...
func main() {
	...
	err := events.WaitForEvent(myPtpPod, startTime, timeout, eventFilter, events.WithContainer("cloud-event-proxy"))
	if err != nil {
