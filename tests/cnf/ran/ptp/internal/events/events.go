...
var (
	...
	containsEventRegexp = regexp.MustCompile(`msg="(received event|sent event|Got CurrentState:)`)
...
// error if no event matches the provided filters within the timeout period. The filters are applied in a logical AND
// operation, meaning that all filters must match for the event to be considered a match.
//
...
func WaitForEvent(
	...
	return wait.PollUntilContextTimeout(
		context.TODO(), 5*time.Second, timeout, true, func(ctx context.Context) (bool, error) {
			...
			extractedEvents := extractEventsFromLogs(logs)
...
func extractEventsFromLogs(logs []byte) []event.Event {
	var extractedEvents []event.Event

	for line := range bytes.Lines(logs) {
		if !containsEventRegexp.Match(line) {
			continue
		}
		...
		// First convert the escaped newlines to actual newlines.
		eventJSON = bytes.ReplaceAll(eventJSON, []byte(`\n`), []byte("\n"))

		// Next remove all the backslashes from places like quotes. This will fail if the JSON is expected to
		// have backslashes, but this should not happen with events currently.
		eventJSON = bytes.ReplaceAll(eventJSON, []byte(`\`), []byte(``))
		...
		err := json.Unmarshal(eventJSON, &extractedEvent)
