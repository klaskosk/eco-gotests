...
func extractEventsFromLogs(logs []byte) []event.Event {
	...
	for line := range bytes.Lines(logs) {
		...
		// Removing all the backslashes is good enough to make the JSON valid. This will fail if the JSON is
		// expected to have backslashes, but this should not happen with events currently.
