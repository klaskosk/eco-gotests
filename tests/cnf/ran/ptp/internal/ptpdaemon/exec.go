...
func ExecuteCommandInPtpDaemonPod(
	...
	for retry := range execOptions.attempts {
		...
		if execOptions.retryOnError && err != nil {
			glog.V(tsparams.LogLevel).Infof("Failed to execute command `%v` in PTP daemon pod on node %s: %v",
		...
		if execOptions.retryOnEmptyOutput && output.Len() == 0 {
			glog.V(tsparams.LogLevel).Infof("Failed to execute command `%v` in PTP daemon pod on node %s: no output returned",
				command, nodeName)
	...
	return "", fmt.Errorf(
		"failed to execute command `%v` in PTP daemon pod on node %s after %d attempts: ran out of attempts",
