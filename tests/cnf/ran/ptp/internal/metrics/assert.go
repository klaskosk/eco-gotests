...
func AssertQuery[V constraints.Integer](
	...
	// lastTime is the time at which the query will stop being executed. It is the start time plus the timeout. If
	// queryTime is lastTime or later, the query is considered to have timed out.
	lastTime := opts.startTime.Add(opts.timeout)
	...
	for queryTime.Before(lastTime) || queryTime.Equal(lastTime) {
		...
		case <-time.After(time.Until(queryTime)):
			...
			// queryTime is allowed to be in the past and should be to avoid it being in the future.
...
// actual map or it is found but with a different value, the assertion fails.
//
// SECURITY: This function does not perform any sort of sanitization on the query. It should only be used with trusted
// queries.
func AssertThresholds(
	...
	for profile, expectedThreshold := range expected {
		...
		if expectedThreshold != actualThreshold {
			return fmt.Errorf("expected threshold for profile %s to be %v, but got %v",
				profile, expectedThreshold, actualThreshold)
		}
	}
...
func assertQueryAtTime[V constraints.Integer](
	...
	if err != nil {
		return fmt.Errorf("failed to execute query at time %s: %w", assertTime, err)
	}
