...
func SetHoldOverTimeouts(
	client *clients.Settings, profiles []*ProfileInfo, holdoverTimeout int64) (map[ProfileReference]*int64, error) {
	oldHoldovers := make(map[ProfileReference]*int64, len(profiles))
	...
	for _, profile := range profiles {
		...
		if ptpConfig.Definition.Spec.Profile[profileIndex].PtpClockThreshold == nil {
			...
			oldHoldovers[profile.Reference] = &ptpConfig.Definition.Spec.Profile[profileIndex].PtpClockThreshold.HoldOverTimeout
		}
	...
	glog.V(tsparams.LogLevel).Infof(
		"Set holdover timeout to %d minutes while saving original values: %#v", holdoverTimeout, oldHoldovers)
...
func WaitForHoldOverTimeouts(
	...
	glog.V(tsparams.LogLevel).Infof(
		"Waiting for holdover timeout to be set to %d minutes for profiles on node %s",
...
func ResetHoldOverTimeouts(client *clients.Settings, oldHoldovers map[ProfileReference]*int64) error {
	...
	for reference, oldHoldover := range oldHoldovers {
		...
		glog.V(tsparams.LogLevel).Infof(
			"Resetting holdover timeout for profile %s to %d from %d",
			reference.ProfileName, oldHoldover, ptpConfig.Definition.Spec.Profile[reference.ProfileIndex].PtpClockThreshold.HoldOverTimeout)
...
func WaitForOldHoldOverTimeouts(
	prometheusAPI prometheusv1.API,
	nodeName string,
	oldHoldovers map[ProfileReference]*int64,
	timeout time.Duration) error {
	glog.V(tsparams.LogLevel).Infof(
