...
// DeployV1ConsumersOnWorkers deploys the cloud-event-consumer deployment and service on all worker nodes in the
...
// Next, it lists the worker nodes and deploys the consumer on each node using [deployV1ConsumerOnNode].
func DeployV1ConsumersOnWorkers(client *clients.Settings) error {
	...
	for _, worker := range workers {
		...
		deployErrors = append(deployErrors, deployV1ConsumerOnNode(client, worker.Definition.Name, images))
	}

	return errors.Join(deployErrors...)
}

// CleanupV1ConsumersOnWorkers deletes the cloud-event-consumer deployment and service on all worker nodes in the
// cluster, assuming the event API version is v1.
//
// It works by first listing the worker nodes and cleaning up the consumer on each node using [cleanupV1ConsumerOnNode].
// Next, it cleans up the cluster-level resources created by [deployV1ConsumerOnCluster] using
// [cleanupV1ConsumerOnCluster].
func CleanupV1ConsumersOnWorkers(client *clients.Settings) error {
	workers, err := nodes.List(client, metav1.ListOptions{
		LabelSelector: labels.SelectorFromSet(labels.Set(RANConfig.WorkerLabelMap)).String(),
	})
	if err != nil {
	...
	for _, worker := range workers {
		cleanupErrors = append(cleanupErrors, cleanupV1ConsumerOnNode(client, worker.Definition.Name))
	}
...
// deployV1ConsumerOnNode deploys all the resources required for a consumer on a specific worker node. It creates the
// following resources in order:
//
...
// cleanupV1ConsumerOnNode removes all the resources created by deployV1ConsumerOnNode on a specific worker node. It
// deletes the following resources in order:
//
...
func createV1ConsumerDeploymentOnNode(client *clients.Settings, nodeName string, images *eventImages) error {
	...
	consumerDeployment := deployment.NewBuilder(
		...
		*consumerContainer,
		...
		WithServiceAccountName(sidecarConsumerSA).
...
func createV1SidecarConsumerServiceAccount(client *clients.Settings) error {
	_, err := serviceaccount.NewBuilder(client, sidecarConsumerSA, tsparams.CloudEventsNamespace).Create()
	if err != nil {
...
func deleteV1SidecarConsumerServiceAccount(client *clients.Settings) error {
	serviceAccountBuilder, err := serviceaccount.Pull(client, sidecarConsumerSA, tsparams.CloudEventsNamespace)
	if err != nil {
		return nil
	}
...
func createV1SidecarConsumerClusterRole(client *clients.Settings) error {
	...
	_, err := rbac.NewClusterRoleBuilder(client, sidecarConsumerRole, rules[0]).WithRules(rules[1:]).Create()
	if err != nil {
...
func deleteV1SidecarConsumerClusterRole(client *clients.Settings) error {
	cr, err := rbac.PullClusterRole(client, sidecarConsumerRole)
	if err != nil {
		return nil
	}
...
func createV1SidecarConsumerClusterRoleBinding(client *clients.Settings) error {
	subject := rbacv1.Subject{
		Kind:      "ServiceAccount",
		Name:      sidecarConsumerSA,
		Namespace: tsparams.CloudEventsNamespace,
	}

	_, err := rbac.NewClusterRoleBindingBuilder(
		client, sidecarConsumerRoleBinding, sidecarConsumerRole, subject).
		Create()
	if err != nil {
...
func deleteV1SidecarConsumerClusterRoleBinding(client *clients.Settings) error {
	crb, err := rbac.PullClusterRoleBinding(client, sidecarConsumerRoleBinding)
	if err != nil {
		return nil
	}
...
func createV1PrometheusRole(client *clients.Settings) error {
	...
	_, err := rbac.NewRoleBuilder(client, prometheusK8s, tsparams.CloudEventsNamespace, rules[0]).
...
func deleteV1PrometheusRole(client *clients.Settings) error {
	roleBuilder, err := rbac.PullRole(client, prometheusK8s, tsparams.CloudEventsNamespace)
	if err != nil {
		return nil
	}
...
func createV1PrometheusRoleBinding(client *clients.Settings) error {
	...
	_, err := rbac.NewRoleBindingBuilder(
		client, prometheusK8s, tsparams.CloudEventsNamespace, prometheusK8s, subject).
		Create()
	if err != nil {
		return fmt.Errorf("failed to create prometheus role binding: %w", err)
	}
...
func deleteV1PrometheusRoleBinding(client *clients.Settings) error {
	roleBindingBuilder, err := rbac.PullRoleBinding(client, prometheusK8s, tsparams.CloudEventsNamespace)
	if err != nil {
		return nil
	}
