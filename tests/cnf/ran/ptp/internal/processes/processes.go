...
func GetPtp4lPIDsByRelatedProcess(client *clients.Settings, nodeName string, relatedProcess PtpProcess, related bool) ([]string, error) {
	...
	matches := ptpProcessRegex.FindStringSubmatch(output)
	if len(matches) != 3 {
		return nil, fmt.Errorf("failed to get index for process %s: %v", relatedProcess, output)
	}
	...
	getPIDsCommand := fmt.Sprintf("pgrep -a ptp4l | grep %s /var/run/ptp4l.%s.config | cut -d' ' -f1", grepFlag, index)
