...
func GetArgoCdAppGitDetails() error {
	if raninittools.RANConfig.HubKubeconfig != "" {
		for _, app := range tsparams.ArgoCdApps {
			argoCdApp, err := argocd.PullApplication(raninittools.HubAPIClient, app, ranparam.OpenshiftGitops)
			if err != nil {
				return err
			}
...
func SetGitDetailsInArgoCd(
	...
	app, err := argocd.PullApplication(raninittools.HubAPIClient, appName, ranparam.OpenshiftGitops)
	if err != nil {
		return err
	}
...
func waitForArgoCdChangeToComplete(appName string, syncMustBeValid bool, timeout time.Duration) error {
	return wait.PollUntilContextTimeout(
		context.TODO(), tsparams.ArgoCdChangeInterval, timeout, true, func(ctx context.Context) (done bool, err error) {
			...
			for i, condition := range app.Definition.Status.Conditions {
			...
			operationState := app.Definition.Status.OperationState
			...
			statusSource := app.Definition.Status.Sync.ComparedTo.Source
			appSource := app.Definition.Spec.Source
			...
			if statusSource.RepoURL == appSource.RepoURL &&
				...
				if syncMustBeValid {
					return app.Definition.Status.Sync.Status == "Synced", nil
				}
