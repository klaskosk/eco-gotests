...
var _ = BeforeSuite(func() {
	err := ranhelper.InitializeSpokeNames()
	Expect(err).ToNot(HaveOccurred(), "Failed to initialize spoke names")

	err = ranhelper.InitializeVersions()
	Expect(err).ToNot(HaveOccurred(), "Failed to initialize versions for ZTP suite")
