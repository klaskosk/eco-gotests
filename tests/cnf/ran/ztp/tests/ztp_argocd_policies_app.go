...
var _ = Describe("ZTP Argo CD Policies Tests", Label(tsparams.LabelArgoCdPoliciesAppTestCases), func() {
	...
	Context("with an image registry configured on the du profile", func() {
		var imageRegistryConfig *imageregistry.Builder
		const imageRegistryConfigName = "cluster"

		BeforeEach(func() {
			const imageRegistryPath = "/var/imageregistry"

			By("checking if the image registry directory is present on spoke 1")
			_, err := cluster.ExecCommandOnSNO(raninittools.Spoke1APIClient, 3, fmt.Sprintf("ls %s", imageRegistryPath))
			if err != nil {
				Skip(fmt.Sprintf("Image registry directory '%s' should be pre-created but does not exist", imageRegistryPath))
			}

			imageRegistryConfig, err = imageregistry.Pull(raninittools.Spoke1APIClient, imageRegistryConfigName)
		...
		AfterEach(func() {
			...
			err = helper.RestoreImageRegistry(raninittools.Spoke1APIClient, imageRegistryConfigName, imageRegistryConfig)
			...
			err = helper.CleanupImageRegistryConfig(
				raninittools.Spoke1APIClient,
				"image-registry-sc",
				"image-registry-pv-filesystem",
				"image-registry-pvc",
				tsparams.ImageRegistryNamespace)
		...
		It("should validate the image registry exists", reportxml.ID("54354"), func() {
			By("updating Argo CD policies app")
			exists, err := helper.UpdateArgoCdAppGitPath(
			...
			for _, policyName := range tsparams.ImageRegistryPolicies {
				policy, err := helper.WaitForPolicyToExist(
					raninittools.HubAPIClient, policyName, tsparams.TestNamespace, tsparams.ArgoCdChangeTimeout)
			...
			imageRegistryBuilder, err := imageregistry.Pull(raninittools.Spoke1APIClient, imageRegistryConfigName)
			Expect(err).ToNot(HaveOccurred(), "Failed to pull image registry config")

			err = helper.WaitForConditionInImageRegistry(
				imageRegistryBuilder,
				v1.Condition{Type: "Available", Reason: "Ready", Status: "True"},
				tsparams.ArgoCdChangeTimeout)
			Expect(err).ToNot(HaveOccurred(), "Failed to wait for image registry config to be Available")
		})
	})

	Context("Apply and Validate custom source CRs on the du policies", func() {
		AfterEach(func() {
			By("deleting the policy from spoke if it exists")
			policy, err := ocm.PullPolicy(
				raninittools.Spoke1APIClient, tsparams.CustomSourceCrPolicyName, tsparams.TestNamespace)
			if err == nil {
				_, err = policy.Delete()
			...
			serviceAccount, err := serviceaccount.Pull(
				raninittools.Spoke1APIClient, tsparams.CustomSourceCrName, tsparams.CustomSourceTestNamespace)
			if err == nil {
			...
			customNamespace, err := namespace.Pull(raninittools.Spoke1APIClient, tsparams.CustomSourceCrName)
			if err == nil {
			...
			storageClass, err := storage.PullClass(raninittools.Spoke1APIClient, tsparams.CustomSourceStorageClass)
			if err == nil {
		...
		It("verifies new CR kind that does not exist in ztp container image can be created via custom source-cr",
			reportxml.ID("61978"), func() {
				...
				policy, err := helper.WaitForPolicyToExist(
					raninittools.HubAPIClient, tsparams.CustomSourceCrPolicyName, tsparams.TestNamespace, tsparams.ArgoCdChangeTimeout)
		...
		When("ZTP verion is at least 4.14", func() {
			BeforeEach(func() {
			...
			It("should validate the same source cr file name", reportxml.ID("62260"), func() {
				By("updating Argo CD policies app")
				exists, err := helper.UpdateArgoCdAppGitPath(
				...
				_, err = namespace.Pull(raninittools.Spoke1APIClient, tsparams.CustomSourceCrName)
			...
			It("verifies a proper error is returned in ArgoCD app when a non-existence source-cr is used in PGT",
				reportxml.ID("63516"), func() {
					By("updating Argo CD policies app")
					exists, err := helper.UpdateArgoCdAppGitPath(
			...
			It("should validate the source CR search path implemented on a per source file name", reportxml.ID("64407"), func() {
				By("checking service account does not exist on spoke")
				_, err := serviceaccount.Pull(
					raninittools.Spoke1APIClient, tsparams.CustomSourceCrName, tsparams.CustomSourceTestNamespace)
				Expect(err).To(HaveOccurred(), "Service account already exists before test")
				...
				_, err = storage.PullClass(raninittools.Spoke1APIClient, tsparams.CustomSourceStorageClass)
