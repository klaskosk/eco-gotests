...
var _ = Describe("ZTP Spoke Checker Tests", Label(tsparams.LabelSpokeCheckerTests), func() {
	...
	When("a TunedPerformancePatch.yaml PGT disables the chronyd service", func() {
		...
		It("should disable the chronyd service on spoke", reportxml.ID("54237"), func() {
			...
			status, err := cluster.ExecCommandOnSNO(
				raninittools.Spoke1APIClient, 3, "systemctl is-active chronyd.service | cat -")
			...
			status, err = cluster.ExecCommandOnSNO(
				raninittools.Spoke1APIClient, 3, "systemctl is-enabled chronyd.service | cat -")
		...
		It("should not have pods in the network diagnostics namespace", reportxml.ID("60904"), func() {
			pods, err := pod.List(raninittools.Spoke1APIClient, tsparams.NetworkDiagnosticsNamespace)
		...
		It("should not have pods in the console namespace", reportxml.ID("60905"), func() {
			pods, err := pod.List(raninittools.Spoke1APIClient, tsparams.ConsoleNamespace)
