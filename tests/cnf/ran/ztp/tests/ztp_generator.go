...
var _ = Describe("ZTP Generator Tests", Label(tsparams.LabelGeneratorTestCases), func() {
	...
	BeforeEach(func() {
		...
		if _, err = os.Stat(siteConfigPath); err != nil {
			Skip(fmt.Sprintf("could not find site config repo at '%s'", siteConfigPath))
		}
	})
		...
		reportxml.ID("54355"), func() {
			...
			if ranparam.ZtpVersion != "" {
				...
				if err != nil || brew == "" {
					...
					cmd := fmt.Sprintf(
						"skopeo list-tags docker://%s | grep %s", raninittools.RANConfig.ZtpSiteGenerateImage, ranparam.ZtpVersion) +
					...
					cmd := "brew list-builds --package=ztp-site-generate-container --state=COMPLETE --quiet" +
						fmt.Sprintf(" | grep %s", ranparam.ZtpVersion) +
			...
			_, err := cluster.ExecLocalCommand(
				...
				fmt.Sprintf("%s:%s", raninittools.RANConfig.ZtpSiteGenerateImage, ztpImageTag),
				"generator",
				"install",
				"-E",
				"/resources/")
