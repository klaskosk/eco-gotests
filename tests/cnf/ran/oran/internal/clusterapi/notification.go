...
//
// A notification matches when the subscription ID agrees, both prior and post object state are
// present, the cluster is referenced in objectRef or via nodeClusterId/name in either state, and
// postObjectState.extensions contains the given key=value.
func MatchModifyNodeCluster(
	subscriptionID, nodeClusterID uuid.UUID,
