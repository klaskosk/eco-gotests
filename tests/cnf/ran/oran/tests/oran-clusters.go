...
var _ = Describe("ORAN Cluster API Tests", Label(tsparams.LabelPostProvision, tsparams.LabelClusterAPI), func() {
	...
	It("lists node cluster types", reportxml.ID("89869"), func() {
		...
		expectedKeys, err := clusterapi.CollectNodeClusterTypeKeys(HubAPIClient)
		...
		for key := range expectedKeys {
			typeIdx := slices.IndexFunc(apiTypes, func(nodeType oranapi.NodeClusterType) bool {
				return nodeType.Name == key.Name()
			})
			Expect(typeIdx).ToNot(Equal(-1), "Missing NodeClusterType for %s", key.Name())
			Expect(apiTypes[typeIdx].NodeClusterTypeId).ToNot(Equal(uuid.Nil),
				"NodeClusterType %s should have nodeClusterTypeId populated", key.Name())
			Expect(apiTypes[typeIdx].Description).ToNot(BeEmpty(),
	...
	It("gets a specific node cluster type", reportxml.ID("89870"), func() {
		...
		for _, cluster := range eligible {
			key, keyErr := clusterapi.NodeClusterTypeKeyFromManagedCluster(cluster)
			...
			matchingClusters++
			verifyErr := clusterapi.VerifyNodeClusterTypeMatchesKey(retrieved, key)
	...
	It("lists node clusters", reportxml.ID("89872"), func() {
		...
		for _, cluster := range eligible {
			...
			verifyErr := clusterapi.VerifyNodeClusterMatchesManagedCluster(
			...
			if len(apiClusters[clusterIdx].ClusterResourceIds) > 0 {
				verifyErr = clusterapi.VerifyClusterResourceIDsExist(
	...
	It("gets a specific node cluster", reportxml.ID("89873"), func() {
		...
		spoke, err := clusterapi.FindSpokeEligibleManagedCluster(HubAPIClient, RANConfig.Spoke1Name)
		Expect(err).ToNot(HaveOccurred(), "Failed to find an eligible spoke ManagedCluster")

		nodeClusterID, err := clusterapi.ManagedClusterID(spoke)
		...
		verifyErr := clusterapi.VerifyNodeClusterMatchesManagedCluster(retrieved, spoke, apiTypes)
		Expect(verifyErr).ToNot(HaveOccurred(),
	...
	It("filters node clusters by name", reportxml.ID("89874"), func() {
		...
		spokeName := spoke.Definition.Name
		spokeID, err := clusterapi.ManagedClusterID(spoke)
		Expect(err).ToNot(HaveOccurred(), "Failed to parse ManagedCluster clusterID")

		By("filtering NodeClusters by name equals the spoke name")

		eqClusters, err := clusterClient.ListNodeClusters(oranapi.WithFilter(filter.Equals("name", spokeName)))
		Expect(err).ToNot(HaveOccurred(), "Failed to filter NodeClusters by name equals")
		Expect(eqClusters).To(HaveLen(1), "Filter name=%s should return exactly one NodeCluster", spokeName)
		Expect(eqClusters[0].Name).To(Equal(spokeName), "Filtered NodeCluster name should match")
		...
		neqClusters, err := clusterClient.ListNodeClusters(oranapi.WithFilter(filter.DoesNotEqual("name", spokeName)))
		...
		containsSpoke := slices.ContainsFunc(neqClusters, func(nodeCluster oranapi.NodeCluster) bool {
			return nodeCluster.Name == spokeName
		})
	...
	It("lists cluster resource types", reportxml.ID("89876"), func() {
		...
		expectedKeys, err := clusterapi.CollectClusterResourceTypeKeys(HubAPIClient)
		...
		for key := range expectedKeys {
			typeIdx := slices.IndexFunc(apiTypes, func(resourceType oranapi.ClusterResourceType) bool {
				return resourceType.Name == key.Name()
			})
			Expect(typeIdx).ToNot(Equal(-1), "Missing ClusterResourceType for %s", key.Name())
			verifyErr := clusterapi.VerifyClusterResourceTypeMatchesKey(apiTypes[typeIdx], key)
			Expect(verifyErr).ToNot(HaveOccurred(),
	...
	It("gets a specific cluster resource type", reportxml.ID("89877"), func() {
		...
		for _, agent := range agents {
			key := clusterapi.ClusterResourceTypeKeyFromAgent(agent.Definition)
			if key.Name() != retrieved.Name {
				continue
			}

			matchingAgents++
			verifyErr := clusterapi.VerifyClusterResourceTypeMatchesKey(retrieved, key)
			Expect(verifyErr).ToNot(HaveOccurred(),
	...
	It("lists cluster resources", reportxml.ID("89878"), func() {
		...
		for _, agent := range agents {
			matched, matchErr := clusterapi.FindClusterResourceForAgent(apiResources, agent.Definition)
			Expect(matchErr).ToNot(HaveOccurred(),
				"Agent %s missing from API response", clusterapi.ExpectedAgentExternalID(agent.Definition))

			verifyErr := clusterapi.VerifyClusterResourceMatchesAgent(matched, agent.Definition)
			Expect(verifyErr).ToNot(HaveOccurred(),
				"Agent %s does not match API ClusterResource",
				clusterapi.ExpectedAgentExternalID(agent.Definition))
		}
	})
	...
	It("gets a specific cluster resource", reportxml.ID("89879"), func() {
		...
		for _, agent := range agents {
			_, matchErr := clusterapi.FindClusterResourceForAgent(
			...
			matched = true
			verifyErr := clusterapi.VerifyClusterResourceMatchesAgent(retrieved, agent.Definition)
			Expect(verifyErr).ToNot(HaveOccurred(),
	...
	It("creates a cluster subscription", reportxml.ID("89880"), func() {
		...
		DeferCleanup(func() {
			err := clusterClient.DeleteClusterSubscription(*created.SubscriptionId)
