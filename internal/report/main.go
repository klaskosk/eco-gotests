...
func getTrees(branch string) (map[CacheKey]*SuiteTree, error) {
	...
	if branch != "" {
		...
		treeMap, err = getLocalTreeMap(ctx, cache, ".")
	}
...
func getLocalTreeMap(ctx context.Context, cache *Cache, repoPath string) (map[CacheKey]*SuiteTree, error) {
	tree, err := getFromCacheOrDryRun(ctx, cache, ".")
	if err != nil {
		return nil, err
	}

	key, err := cache.GetKeyFromPath(".")
	if IsMiss(err) {
		treeMap := map[CacheKey]*SuiteTree{
			CacheKey{Branch: "local", Revision: "local"}: tree,
		}
...
func getFromCacheOrClone(ctx context.Context, cache *Cache, patterns []string) (map[CacheKey]*SuiteTree, error) {
	...
	for key, tree := range treeMap {
		...
		tree, err = getFromCacheOrDryRun(ctx, cache, repoPath)
		if err != nil {
			return nil, err
		}
...
func getFromCacheOrDryRun(ctx context.Context, cache *Cache, repoPath string) (*SuiteTree, error) {
	tree, err := cache.GetOrCreate(repoPath, func() (*SuiteTree, error) {
		reportPath, err := DryRun(ctx, repoPath)
		if err != nil {
