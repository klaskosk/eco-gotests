# Test Case Summary for 80502

Test case 80502 is located in tests/cnf/ran-deployment/deploymenttypes/tests/deployment-types.go and is named "checks if policy type is ACMPG".

## Goal

The goal of this test case is to verify if the policy type is ACMPG.
...
2. If `policyTemplate` is not `ACMPG`, skip the test.
3. Log that the policy type is `ACMPG`.
