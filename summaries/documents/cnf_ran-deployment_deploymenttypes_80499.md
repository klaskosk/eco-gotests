# Test Case Summary for 80499

Test case 80499 is located in tests/cnf/ran-deployment/deploymenttypes/tests/deployment-types.go and is named "checks if cluster type is ThreeNode".

## Goal

The goal of this test case is to verify if the cluster type is ThreeNode.
...
2. If `clusterKind` is not `ThreeNode`, skip the test.
3. Log that the cluster type is `ThreeNode`.
