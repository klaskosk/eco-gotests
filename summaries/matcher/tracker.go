...
func LoadTracker(filePath string) (*DocumentTracker, error) {
	...
	if _, err := os.Stat(filePath); os.IsNotExist(err) {
		glog.V(90).Infof("Tracking file %q does not exist, starting with empty tracker", filePath)
	...
	glog.V(90).Infof("Loaded %d tracked documents from %q", len(documentIDs), filePath)
...
func (dt *DocumentTracker) Save() error {
	...
	glog.V(90).Infof("Saved %d tracked documents to %q", len(documentIDs), dt.filePath)
